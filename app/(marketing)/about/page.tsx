  ...
  return (
    <div className="bg-black text-white overflow-hidden">
      ...
      <div className="container mx-auto px-4 md:px-8 pt-28 md:pt-32">
        <p className="about-serif italic text-sm text-white/40 tracking-wide">
          main / about us
        </p>
      </div>

      {/* ── Hero: giant headline + overlapping images ── */}
      <section className="relative container mx-auto px-4 md:px-8 pb-20 md:pb-32">
        <div className="relative flex items-center justify-center min-h-[65vh] md:min-h-[80vh] lg:min-h-[85vh]">
          {/* Main center image — tall, behind headline */}
          <motion.div
            initial={{ opacity: 0, y: 40 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.9, delay: 0.2 }}
            className="absolute z-10 top-[2%] md:top-0 left-1/2 -translate-x-1/2 w-[min(72vw,320px)] md:w-[420px] lg:w-[480px] h-[min(85vw,400px)] md:h-[560px] lg:h-[620px] overflow-hidden border border-white/10 shadow-[0_0_80px_rgba(255,140,66,0.12)]"
          >
            <img
          ...
          {/* Left — developer coding, B&W */}
          <motion.div
            initial={{ opacity: 0, x: -30 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.7, delay: 0.35 }}
            className="absolute z-20 top-[10%] md:top-[8%] left-[0%] md:left-[4%] lg:left-[8%] w-[clamp(120px,22vw,240px)] aspect-square overflow-hidden border border-white/15 shadow-[0_20px_60px_rgba(0,0,0,0.6)]"
          >
            <img
          ...
          <motion.div
            ...
            className="absolute z-20 top-[14%] md:top-[12%] right-[0%] md:right-[4%] lg:right-[8%] w-[clamp(110px,20vw,220px)] aspect-square overflow-hidden border border-[#FF8C42]/20 shadow-[0_20px_60px_rgba(255,140,66,0.15)]"
          >
            <img
          ...
          {/* Bottom-left accent — code matrix */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.7, delay: 0.55 }}
            className="absolute z-20 bottom-[18%] md:bottom-[14%] left-[8%] md:left-[14%] w-[clamp(100px,18vw,200px)] aspect-[4/3] overflow-hidden border border-lime/20 shadow-[0_20px_50px_rgba(190,242,100,0.1)] hidden sm:block"
          >
            <img
            ...
            />
            <div className="absolute inset-0 bg-lime/5 mix-blend-overlay pointer-events-none" />
          </motion.div>
          ...
          <motion.h1
            ...
            className="relative z-30 text-[clamp(3.5rem,14vw,11rem)] font-bold lowercase leading-[0.85] tracking-tight text-center select-none w-full drop-shadow-[0_4px_30px_rgba(0,0,0,0.9)]"
          >
            about us
          </motion.h1>
        </div>
      ...
      <section className="container mx-auto px-4 md:px-8 mb-20 md:mb-32 max-w-5xl">
        <motion.p
          ...
          className="text-2xl md:text-4xl lg:text-[2.75rem] font-bold leading-snug tracking-tight text-center lowercase"
        >
      ...
      {/* ── Process section with winding path ── */}
      <section className="container mx-auto px-4 md:px-8 pb-24 md:pb-40 max-w-3xl relative">
        <motion.p
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
          className="about-serif italic text-lime text-lg md:text-xl mb-16 md:mb-20"
        >
          process.
        </motion.p>
        ...
        <div className="space-y-24 md:space-y-32 relative">
          {PROCESS_STEPS.map((step, i) => (
            ...
            >
              {/* Dot on path — desktop only */}
              <div className={`hidden md:block absolute top-2 ${step.side === 'left' ? 'right-[42%]' : 'left-[42%]'} w-2.5 h-2.5 rounded-full bg-lime about-step-dot`} />

              <div className="max-w-xs">
                <p className="text-lime font-mono text-sm mb-2">{step.num}.</p>
                <h3 className="text-xl md:text-2xl font-bold lowercase mb-3">{step.title}</h3>
        ...
        <div className="container mx-auto px-4 md:px-8 grid grid-cols-2 md:grid-cols-4 gap-8 max-w-4xl">
          {[
            ...
            <div key={s.label} className="text-center">
              <p className="text-2xl md:text-3xl font-bold font-mono text-[#FF8C42]">{s.value}</p>
      ...
      <section className="container mx-auto px-4 md:px-8 py-20 md:py-28 flex flex-col items-center text-center">
        <motion.div
          ...
          className="space-y-6"
        >
          <p className="about-serif italic text-white/40 text-lg">ready to join us?</p>
          <h2 className="text-3xl md:text-5xl font-bold lowercase tracking-tight">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
            <Link href="/get-started">
              <Button
                variant="outline"
                className="rounded-full border-white/30 bg-transparent text-white hover:bg-white hover:text-black px-10 h-12 text-sm tracking-wide"
              >
                get in touch
              </Button>
            </Link>
            <Link href="/contests">
              <Button className="rounded-full bg-gradient-to-r from-[#FF8C42] to-[#FFB088] text-black hover:opacity-90 px-10 h-12 border-0 text-sm">
                browse contests
              </Button>
            </Link>
          </div>
